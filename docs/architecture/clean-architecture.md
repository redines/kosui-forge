...
The enforcement cannot rely on a denylist containing only `os`, `shutil`, `subprocess`, and `tempfile`: standard-library filesystem access through `open`, `pathlib` I/O methods, `fileinput`, `glob`, or `mmap` is equally forbidden in inner layers. Literal calls such as `importlib.import_module("kosui_forge.infrastructure.cli")` are dependency edges and must not evade layer or cycle checks. Non-literal dynamic import targets are prohibited in `kosui_forge` layer code because their dependency direction cannot be proven statically.

The `allowed` fixtures prove that explicit inward imports, third-party adapters, UI presentation libraries, and composition-root wiring are accepted. The `forbidden` fixtures deliberately cover Qt, GitHubKit, keyring, compatibility imports, process/filesystem I/O, absolute and relative outward imports, literal dynamic imports, package-facade shortcuts, and presentation bypasses. Cycle fixtures cover both absolute and relative cycles. Each category needs a relationship assertion proving that the intended violation was found; merely finding some violation in the same fixture is insufficient.
